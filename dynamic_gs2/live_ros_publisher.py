...
class LivePublisher:
    ...
    def _finger_world_points(self):
        """The two gripper finger-pad world points the last mask render stashed, as two (3,) float64
        arrays for the SHM frame (zeros == unavailable). Read AFTER the mask render completed (the
        _mask_gen attribute it sets), so they match the mask/pose of the frame being written."""
        zeros = np.zeros(3, dtype=np.float64)
        if self._mask_gen is None:
            return zeros, zeros
        fl, fr = getattr(self._mask_gen, "last_finger_world", (None, None))
    ...
    def _process_synced_pair(self, image_msg: CompressedImage, depth_msg: Image) -> None:
        ...
        if self._new_layout:
            ...
            _fl, _fr = self._finger_world_points()
            self._fc.write_frame(self.shm.buf, self._nl_layout, self._fc.Frame(
