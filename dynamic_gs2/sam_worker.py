...
DETACH NOTE (dynamic_gs2): copied verbatim from dynamic_gs.utils.sam_worker (the
worker path-loads its sibling helpers `sam3d.py` / `fastsam_segmentation.py` from
this same dir, so those are colocated copies). `sam3_segmentation.py` is NOT copied
— gs2's `load_sam3`/`sam3_infer` handlers have zero callers on the live path. If
SAM3 is ever wired back into dynamic_gs2, copy `sam3_segmentation.py` (+360 LOC) as
a sibling too.
...
class SamWorkerClient:
    ...
    def __init__(self,
        ...
        # (Detach: the PYTHONPATH=scripts/ inject was removed — the worker never
        #  package-imports dynamic_gs.*; it only path-loads sibling .py files via
        #  _load_sibling_module, so no repo root on sys.path is needed.)
