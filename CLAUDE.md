...
### Sim-to-real ZED-X depth-noise model on sim depth (2026-06-15, IMPLEMENTED, OFF by default)

The Gazebo `libgazebo_ros_openni_kinect.so` depth sensor returns the GPU z-buffer **noise-free** (its URDF `<noise>` block applies to RGB only, not depth) — so sim depth is perfect, the sim-to-real gap. New [`dynamic_gs/utils/zed_depth_noise.py`](dynamic_gs/utils/zed_depth_noise.py) corrupts clean sim depth into ZED-X-realistic depth, paper-faithful to **Ortiz, Cabrera, Gonçalves, "Depth Data Error Modeling of the ZED 3D Vision Sensor from Stereolabs", ELCVIA 17(1), 2018**:

* **Axial exp noise** (their Eq. 8): per-pixel zero-mean Gaussian, `σ(z) = a·exp(b·z)` metres. All four ZED resolutions are in the module's `_ZED_RES` table (Table 2 fits + §4.1 null-rates); pick via `DGS_SIM_ZED_RES` ∈ {hd2k, hd1080, hd720, wvga}, default **hd1080** (`a=0.0106, b=0.2215`, R²=0.96) — closest to our recorded HD1200. hd1080 gives σ ≈ 11.8 mm @0.5 m → 16.5 mm @2 m → 20.6 mm @3 m (validated realized σ matches within 0.1 mm at 0.5–2 m).
* **Random holes** — per-resolution null-pixel rate (hd1080 2.43%, §4.1). **Range gate** [0.05, 3.0] m matches `DEPTH_MIN_M/MAX_M` in [`online_fusion.py`](dynamic_gs/utils/online_fusion.py).
* **NOT modeled** (deliberate, "paper-faithful" decision): edge/discontinuity flying-pixels — the paper measured a flat checkerboard, no edge term.

**Inject point:** [`live_ros_publisher.py`](dynamic_gs/utils/live_ros_publisher.py) `_process_synced_pair`, right after `_decode_raw_depth` returns float32 metres and BEFORE the SHM write + disk write — so both the live SHM stream and recorded TIFFs get the noisy depth. Loaded via importlib (the publisher runs as a standalone script in `dynamic_gs_ros`, not as a package); module uses only numpy+os so it's safe in that minimal env. **Master toggle `DGS_SIM_ZED_NOISE=1` (OFF by default).** Knobs env-overridable: `DGS_SIM_ZED_A/B`, `DGS_SIM_ZED_HOLE_RATE`, `DGS_SIM_ZED_Z_MIN/Z_MAX`. The exp `f(Z)` is the paper's empirical RMS fit, NOT the fundamental `σ_z ∝ z²` stereo physics — fine over the 0.5–3 m working range.
