...
* **Shipped**: [`scripts/adaptive_downsample.py`](scripts/adaptive_downsample.py). Keep all points within **1.0 m of the last camera pose** at native 1.5 mm density. Voxel-downsample the rest to **5 mm**. On validate_run_1: 10.15 M → 1.14 M (8.9× reduction, near zone = 3.4 % of points).
* Hyperparameters hardcoded in the script: `NEAR_RADIUS_M = 1.0`, `FAR_VOXEL_M = 0.005`. Not yet wired into `capture_only.py` / `live_session.py` — currently a manual post-step.
