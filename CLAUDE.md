...
* **Axial noise** (stereo physics `σ_z ∝ z²`): per-pixel zero-mean Gaussian, `σ_z(z) = σ0 + k·z²` metres. **Measured on `ZED/zed_validate2`** (HD1200 NEURAL conf=50): residual std to a local plane over 41,927 flat 9×9 windows / 30 frames → 0.29 mm @0.4 m, 0.48 mm @1.0 m, 2.15 mm @1.9 m. LSQ fit → `σ0≈0` (kept a 0.05 mm quantization floor), **`k=0.477 mm/m²`** (`DGS_SIM_ZED_K_M=0.000477`, `DGS_SIM_ZED_SIGMA0_M=0.00005`). So σ is **SUB-mm out to ~1.5 m, ~1.7 mm @1.9 m**. Visible point spread @0.5 m ≈ 0.8 mm.
* **Random holes** ~1% (`DGS_SIM_ZED_HOLE_RATE`). **Range gate** [0.05, 3.0] m matches `DEPTH_MIN_M/MAX_M` in [`online_fusion.py`](dynamic_gs/utils/online_fusion.py).
